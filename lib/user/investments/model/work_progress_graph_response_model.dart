...
class WorkProgressGraphResponseModel {
  const WorkProgressGraphResponseModel({
  ...
  final List<WorkProgressGraphPointModel> data;
  final int code;

  factory WorkProgressGraphResponseModel.fromJson(Map<String, dynamic> json) {
    final rawData = json['data'] ?? json['Data'];
    final list = rawData is List ? rawData : const [];

    return WorkProgressGraphResponseModel(
      status: _readStatus(json['status'] ?? json['Status']),
      message: (json['message'] ?? json['Message'] ?? '').toString(),
      data: list
          .whereType<Map>()
          .map(
            (item) => WorkProgressGraphPointModel.fromJson(
